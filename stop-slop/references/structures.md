...
## Subordinate Clause Chains

AI strings together "which," "that," and "who" clauses instead of using cumulative sentence structure. This creates lateral sidebars that kill momentum.

| Pattern                                                                  | Fix                                                                               |
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| "X that Y, which Z, who W"                                               | Rewrite as cumulative: base clause + free modifying phrases                       |
| "The tool that handles leads, which scores them, that routes to reps"    | "The tool handles leads, scoring each one, routing winners to reps"               |
| "She sent the email that contained the proposal, which outlined pricing" | "She sent the email, the proposal tucked inside, pricing laid out in three tiers" |

**Instead:** Write cumulative sentences. Short base clause, then participial phrases (-ing, -ed) and absolute phrases that zoom into detail. Each modifier narrows the camera.

## Uniform Sentence Length

AI writes sentences of nearly identical length (15-25 words), creating a metronomic rhythm that is the single most detectable structural pattern.

| Pattern                                   | Fix                                                                        |
| ----------------------------------------- | -------------------------------------------------------------------------- |
| Four sentences all 15-20 words            | Vary radically: 4-word fragment, 40-word cumulative, balanced doublet      |
| Every sentence starts subject-verb-object | Use 1-2 suspensive sentences per piece: "If...", "When...", "Not until..." |
| No sentence under 6 words or over 30      | Both extremes are necessary for human rhythm                               |

**Instead:** Count word lengths. Three consecutive sentences within 5 words of each other = rewrite one. A 4-word sentence next to a 40-word sentence is rhythm, not a mistake.

## Missing Periodic Structure

AI front-loads the main clause in every sentence. Humans delay it for tension using periodic (suspensive) structure.

| Pattern                                            | Fix                                                            |
| -------------------------------------------------- | -------------------------------------------------------------- |
| "You don't have a pipeline if nobody clicked send" | "If nobody clicked send, you don't have a pipeline"            |
| "The prospect replied after the third email"       | "Not until the third email did the prospect reply"             |
| Always subject-verb-object order                   | Start 1-2 sentences with "If/When/Not until/After/Before/Once" |

**Instead:** Delay the main clause. The condition comes first, the payoff lands last.
