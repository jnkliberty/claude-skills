...
## Reading Replies as Data

Every campaign is a positioning test, not just a meeting machine. Sort replies into three buckets; track all three for two weeks before scaling, rewriting, or dropping an angle.

| Pattern                            | Meaning                        | Action                                           |
| ---------------------------------- | ------------------------------ | ------------------------------------------------ |
| Replies asking for the deliverable | Demand signal                  | Scale the angle; write the segment down          |
| Same objection across segments     | Positioning feedback, not "no" | Fix the framing — cheaper than a new segment     |
| Silence from a whole segment       | Offer may not translate there  | Re-examine offer/segment before blaming the list |

## The 4-Week List Recycle

Decision-makers register almost none of the hundreds of cold emails they receive, so a month later the same email lands fresh for most of the list. A genuinely good offer can rerun on the same list every ~4 weeks and keep pulling steady replies (documented for 2 years straight on one ColdIQ account). A clever-but-weak offer decays instead — novelty has a short shelf life. If a rerun stops pulling, that is offer feedback, not a prompt for new copy.
